...
export function categorizeError(error?: string): string | undefined {
  ...
  if (lower.includes("api error") || lower.includes("http"))
    return "model_error";
  return "unknown";
}
...
export function getStats(): MetadataStats {
  ...
  for (const e of events) {
    ...
    if (e.pipeline.strategy) {
      strategyUsage[e.pipeline.strategy] =
        (strategyUsage[e.pipeline.strategy] || 0) + 1;
    }
  ...
  for (const [model, data] of Object.entries(modelData)) {
    byModel[model] = {
      ...
      success_rate:
        data.queries > 0
          ? Math.round((data.successes / data.queries) * 100) / 100
          : 0,
    };
  }
  ...
  return {
    ...
    streaming: {
      ...
      avg_upgrades:
        streamWithLiquidCount > 0
          ? Math.round((upgradeSum / streamWithLiquidCount) * 100) / 100
          : 0,
      avg_first_response_ms:
        streamWithLiquidCount > 0
          ? Math.round(firstResponseSum / streamWithLiquidCount)
          : 0,
    },
    errors: {
      ...
      model_success_rate:
        totalModelQueries > 0
          ? Math.round((totalModelSuccesses / totalModelQueries) * 100) / 100
          : 0,
    },
  };
}
