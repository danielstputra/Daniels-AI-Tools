...
function runPipeline(opts: {
  ...
  const priorAssistantCount = normalizedMessages.filter(
    (m) => m.role === "assistant",
  ).length;
  ...
  if (opts.autotune && opts.temperature === undefined) {
    autotuneResult = computeAutoTuneParams({
      ...
      overrides: {
        ...
        ...(opts.frequency_penalty !== undefined && {
          frequency_penalty: opts.frequency_penalty,
        }),
        ...(opts.presence_penalty !== undefined && {
          presence_penalty: opts.presence_penalty,
        }),
        ...(opts.repetition_penalty !== undefined && {
          repetition_penalty: opts.repetition_penalty,
        }),
      },
...
completionsRoutes.post("/completions", async (req, res) => {
  ...
  try {
    ...
    if (isUltraplinian) {
      ...
      if (
        tierOverride &&
        ["fast", "standard", "ultra"].includes(tierOverride)
      ) {
