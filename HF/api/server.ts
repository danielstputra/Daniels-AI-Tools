...
app.get("/v1/info", (_req, res) => {
  res.json({
    ...
    version: "0.4.0",
    description:
      "OpenAI-compatible API for ULTRAPLINIAN multi-model racing. Use any OpenAI SDK — just swap the base_url.",
    license: "AGPL-3.0",
    quickstart: {
      python: [
    ...
    models: {
      ...
      "<any-openrouter-model>":
        "Single-model with DANIELS AI pipeline (e.g. openai/gpt-4o)",
    },
    endpoints: {
      "POST /v1/chat/completions":
        "OpenAI-compatible — works with any SDK (model routing built in)",
      "GET  /v1/models": "OpenAI-compatible — list available models",
      "POST /v1/ultraplinian/completions":
        "Raw ULTRAPLINIAN with Liquid Response SSE (power users)",
      ...
      "GET  /v1/dataset/export":
        "Export the open research dataset (JSON or JSONL)",
      "GET  /v1/metadata/stats":
        "ZDR usage analytics (models, latency, pipeline stats — no content)",
      "GET  /v1/metadata/events":
        "Raw metadata event log (paginated, content-free)",
    },
    authentication: {
    ...
    extra_body_options: {
      note: "Pass DANIELS AI-specific options via extra_body in the OpenAI Python SDK",
      options: {
        danielsai: "boolean (default: true) — enable DANIELS AI system prompt",
        autotune: "boolean (default: true) — auto-tune parameters to context",
        strategy:
          "'adaptive'|'precise'|'balanced'|'creative'|'chaotic' (default: adaptive)",
        parseltongue: "boolean (default: true) — obfuscate trigger words",
        stm_modules:
          "string[] (default: ['hedge_reducer','direct_mode']) — post-processing",
        previous_winner: "string — model ID to prioritize in ultraplinian race",
        openrouter_api_key:
          "string — your OpenRouter key (if server does not provide one)",
      },
    },
    limits: {
      requests_total:
        parseInt(process.env.RATE_LIMIT_TOTAL || "5", 10) || "unlimited",
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`
  ╔══════════════════════════════════════════════════════════╗
  ║  DANIELS AI API v0.4.0 — OpenAI-Compatible                 ║
  ...
  ║  QUICKSTART:                                             ║
  ║  from openai import OpenAI                               ║
  ║  c = OpenAI(base_url=".../v1", api_key="daniels_...")        ║
