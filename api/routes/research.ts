 ...
 * Research API Routes
 *
 * Read-access endpoints for the pliny-the-prompter/danielsai HuggingFace
...
researchRoutes.get("/info", (_req, res) => {
  res.json({
    name: "DANIELS AI Research Dataset",
    repo: "pliny-the-prompter/danielsai",
    repo_url: "https://huggingface.co/datasets/pliny-the-prompter/danielsai",
    license: "AGPL-3.0",
    description:
      "Open research dataset for LLM robustness and safety evaluation. Contains opt-in interaction data with full pipeline metadata from ULTRAPLINIAN multi-model racing, AutoTune context-adaptive parameter tuning, Parseltongue obfuscation, and STM output normalization.",
    ...
    schema: {
      dataset_entry: {
        ...
        response: "string",
        autotune:
          "{ strategy, detected_context, confidence, params, reasoning } | null",
        parseltongue:
          "{ triggers_found, technique_used, transformations_count } | null",
        stm: "{ modules_applied: string[] } | null",
        ultraplinian:
          "{ tier, models_queried, winner_model, all_scores, total_duration_ms } | null",
      ...
      metadata_event: {
        ...
        stream: "boolean",
        pipeline:
          "{ danielsai, autotune, parseltongue, stm_modules, strategy }",
        autotune: "{ detected_context, confidence } | null",
        model_results:
          "Array<{ model, score, duration_ms, success, content_length, error_type }> | null",
    ...
    endpoints: {
      ...
      "GET /v1/research/stats":
        "Aggregate stats across all published HF batches",
      "GET /v1/research/batches":
        "List all published batch files in the HF repo",
      "GET /v1/research/batch/*": "Read a specific batch file (JSONL parsed)",
      "GET /v1/research/query":
        "Query the full corpus with filters (category, model, mode, time range)",
      ...
      "GET /v1/research/combined-stats":
        "Combined view: in-memory + published stats",
    },
  });
});
...
researchRoutes.get(/^\/batch\/(.*)$/, async (req, res) => {
  try {
    ...
    if (!filePath || !filePath.endsWith(".jsonl")) {
      res.status(400).json({
        error:
          "Invalid batch path. Expected: metadata/batch_*.jsonl or dataset/batch_*.jsonl",
      });
      return;
    }
...
researchRoutes.post("/flush", tierGate("flush"), async (_req, res) => {
  if (Date.now() - lastFlushTime < 300_000) {
    res
      .status(429)
      .json({ error: "Flush cooldown: please wait 5 minutes between flushes" });
    return;
  }
...
researchRoutes.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="danielsai-corpus-${Date.now()}.jsonl"`,
      );

      // First: stream published batches from HF
      const batches = await listBatchFiles();
      const targetBatches = category
        ? batches.filter((b) => b.category === category)
        : batches;
