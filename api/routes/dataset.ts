...
datasetRoutes.get("/export", (req, res) => {
  ...
  if (format === "jsonl") {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="danielsai-dataset.jsonl"',
    );
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="danielsai-dataset.json"',
    );
    res.json({
      metadata: {
        name: "DANIELS AI Research Dataset",
        description:
          "Opt-in collection of LLM interactions with AutoTune, Parseltongue, and ULTRAPLINIAN pipeline metadata.",
        license: "AGPL-3.0",
        source: "https://huggingface.co/datasets/pliny-the-prompter/danielsai",
