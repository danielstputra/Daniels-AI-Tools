...
consortiumRoutes.post("/completions", async (req, res) => {
  ...
  try {
    ...
    const resolvedOrchestrator: OrchestratorModel =
      orchestrator_model || ORCHESTRATOR_MODELS[0];
    ...
    if (parseltongue) {
      ...
      // Cek apakah ada transformasi yang terjadi (array tidak kosong)
      if (transformed.transformations.length > 0) {
        parseltongueResult = {
          triggers_found: transformed.triggersFound,
          technique_used: parseltongue_technique,
          transformations_count: transformed.transformations.length, // Lebih akurat menggunakan jumlah transformasi
        };

        processedMessages = baseMessages.map((m) => {
          if (m.content === userContent) {
            // Ganti transformed.text menjadi transformed.transformedText
    ...
    if (stream) {
        ...
        queryParams,
        {
          ...
          onModelResult: (result, collected, total) => {
            const score = result.success
              ? scoreResponse(result.content, userContent)
              : 0;
            result.score = score;
      ...
      if (totalSucceeded === 0) {
        sse("consortium:error", {
          error: "All models failed during collection phase",
        });
        res.end();
        return;
      }
      ...
      if (contribute_to_dataset) {
        datasetId = addEntry({
          ...
          ultraplinian: {
            ...
            winner_model: resolvedOrchestrator,
