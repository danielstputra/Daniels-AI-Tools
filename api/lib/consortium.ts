 ...
 * CONSORTIUM Engine
 *
 * The hive-mind mode of DANIELS AI. Instead of racing models to pick a single winner
...
export type OrchestratorModel =
  | (typeof ORCHESTRATOR_MODELS)[number]
  | (string & {});
      ...
      queryModel(model, messages, apiKey, params, controller.signal)
        .then((result) => {
          ...
          if (config.onModelResult) {
            try {
              config.onModelResult(result, settled, models.length);
            } catch {}
          }
        ...
        .catch((err: any) => {
          ...
          if (config.onModelResult) {
            try {
              config.onModelResult(errorResult, settled, models.length);
            } catch {}
          }
...
export async function synthesize(
  ...
  if (!result.success || !result.content) {
    throw new Error(
      `Orchestrator (${orchestratorModel}) failed: ${result.error || "empty response"}`,
    );
  }
