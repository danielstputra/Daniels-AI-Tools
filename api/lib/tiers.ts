 ...
 * Keys are mapped to tiers via DANIELSAI_TIER_KEYS env var.
 *
 * Format:  DANIELSAI_TIER_KEYS="enterprise:sk-ent-xxx,pro:sk-pro-yyy,pro:sk-pro-zzz"
 *
 * Any key in DANIELSAI_API_KEY(S) that isn't in DANIELSAI_TIER_KEYS defaults to "free".
...
if (tierKeyMap.size > 0) {
  ...
  tierKeyMap.forEach((tier) => {
    counts[tier]++;
  });
  console.log(
    `[danielsai] Tier keys loaded: ${counts.enterprise} enterprise, ${counts.pro} pro, ${counts.free} free`,
  );
}
