 ...
 *
 * Reads published batch files back from the pliny-the-prompter/danielsai
...
function getConfig() {
  return {
    ...
    repo: process.env.HF_DATASET_REPO || "pliny-the-prompter/danielsai",
...
async function fetchFileList(prefix?: string): Promise<HFFileEntry[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[HF Reader] List failed (${res.status}): ${await res.text().catch(() => "")}`,
      );
      return [];
    }
    ...
    console.error(
      `[HF Reader] Network error listing files:`,
...
function parseBatchInfos(files: HFFileEntry[]): BatchInfo[] {
  return files
    ...
    .map((f) => {
      const category = f.path.startsWith("metadata/")
        ? ("metadata" as const)
        : ("dataset" as const);
...
export async function readBatch(filePath: string): Promise<unknown[]> {
  ...
  try {
    ...
    batchContentCache.set(filePath, {
      data: records,
      expires: now + BATCH_CONTENT_TTL,
    });

    return records;
  } catch (err) {
    console.error(
      `[HF Reader] Network error reading ${filePath}:`,
      (err as Error).message,
    );
    return [];
  }
  ...
  for (const batch of targetBatches) {
    ...
    for (const record of records) {
      ...
      if (
        ...
        r.timestamp < query.since
      )
        continue;
      if (
        query.until &&
        typeof r.timestamp === "number" &&
        r.timestamp > query.until
      )
        continue;
      if (query.model) {
        const modelMatch =
          r.model === query.model ||
          (r.winner &&
            (r.winner as Record<string, unknown>).model === query.model) ||
          (Array.isArray(r.models_queried) &&
            r.models_queried.includes(query.model));
