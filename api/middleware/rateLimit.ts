...
export function rateLimit(
  ...
  const TOTAL_LIMIT =
    tierConfig?.rateLimit.total ??
    parseInt(process.env.RATE_LIMIT_TOTAL || "5", 10);
  const MINUTE_LIMIT =
    tierConfig?.rateLimit.perMinute ??
    parseInt(process.env.RATE_LIMIT_PER_MINUTE || "60", 10);
  const DAY_LIMIT =
    tierConfig?.rateLimit.perDay ??
    parseInt(process.env.RATE_LIMIT_PER_DAY || "1000", 10);
  ...
  if (TOTAL_LIMIT > 0 && bucket.totalRequests >= TOTAL_LIMIT) {
    ...
    res.status(429).json({
      ...
      upgrade:
        tier === "free"
          ? "Set DANIELSAI_TIER_KEYS to assign a higher tier to your API key."
          : undefined,
    });
    return;
  }
  ...
  const totalRemaining =
    TOTAL_LIMIT > 0 ? TOTAL_LIMIT - bucket.totalRequests : Infinity;
  res.setHeader(
  ...
  res.setHeader(
    ...
    TOTAL_LIMIT > 0 ? totalRemaining : "unlimited",
  );
