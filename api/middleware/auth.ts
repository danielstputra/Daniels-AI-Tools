 ...
 * Keys are loaded from the DANIELSAI_API_KEYS environment variable
 * (comma-separated list) or a single DANIELSAI_API_KEY.
 *
...
function getValidKeys(): string[] | null {
  const multi = process.env.DANIELSAI_API_KEYS;
  if (multi) {
    const keys = multi
      .split(",")
      .map((k) => k.trim())
  ...
  const single = process.env.DANIELSAI_API_KEY;
  if (single) {
    return [single.trim()];
  }
...
if (!getValidKeys()) {
  console.warn(
    "[danielsai] WARNING: No API keys configured (DANIELSAI_API_KEYS / DANIELSAI_API_KEY). " +
