...
By using DANIELS AI -- whether through the web frontend, the API server, or a self-hosted deployment -- you agree to these terms. If you do not agree, do not use the software.

---

## 2. What DANIELS AI Is

DANIELS AI is an open-source research tool for AI safety, red teaming, and cognition research. It is provided **as-is** for experimentation and study. It is not a commercial product, and there is no company behind it -- just an open-source project and its contributors.
...
DANIELS AI collects data in **three tiers**. The first two are always on and collect **only metadata** -- never your messages, prompts, responses, API keys, or any personally identifiable information. The third tier is strictly opt-in and does include conversation content.
...
| Pipeline flags           | Whether danielsai, autotune, parseltongue, and STM were enabled                                                   |
...
| Pipeline config       | Autotune on/off, parseltongue on/off, STM modules, strategy, danielsai on/off                  |
...
| Excluded Data  | Why                                                          |
| -------------- | ------------------------------------------------------------ |
| API keys       | Never stored, never transmitted to DANIELS AI infrastructure |
| IP addresses   | Not recorded                                                 |
...
All collected data (metadata, telemetry, and opt-in dataset entries) is stored in server memory first. Nothing is written to disk on the DANIELS AI server. If the server restarts, in-memory data that has not been published is lost.

### HuggingFace Publishing

When configured (via `HF_TOKEN` and `HF_DATASET_REPO` environment variables), DANIELS AI auto-publishes collected data to a HuggingFace dataset repository.

| Detail             | Value                                                                                  |
| ------------------ | -------------------------------------------------------------------------------------- |
| Default repository | `pliny-the-prompter/danielsai`                                                         |
...
- **Never transmitted to DANIELS AI servers or infrastructure.**
...
Your conversation history is stored **exclusively in your browser's `localStorage`**. DANIELS AI has no account system, no cloud sync, and no server-side storage for your conversations.
...
**DANIELS AI cannot recover lost conversations.** No copy of your chat history exists on any server. You are solely responsible for backing up any conversations you wish to keep. You can export your data manually via your browser's developer tools (`Application → Local Storage`) or by copying conversations from the UI.
...
If you're an individual, a researcher, a student, a hobbyist, a small team, a nonprofit, an educational institution, a government research lab, or a company that doesn't meet the thresholds above -- **this section does not apply to you. DANIELS AI is free for you. Full stop.**

### What Enterprise Users Must Do

Enterprise Users that use the DANIELS AI hosted service (the API server, the web frontend hosted by the project, or any project-operated infrastructure) in any capacity -- including internal tools, employee-facing products, customer-facing products, evaluation, benchmarking, or integration -- must obtain a **Commercial Use License** from the DANIELS AI maintainers.
...
- Authorized use of the hosted DANIELS AI service for enterprise purposes
...
- **Self-hosting DANIELS AI from source code** under the AGPL-3.0 license. The source code license is separate from these service terms. If you clone the repo, run it on your own servers, and comply with the AGPL-3.0 (keep your derivative works open source), you owe nothing.
...
Enterprise Users that access the publicly published research dataset (on HuggingFace) for commercial purposes -- including model training, fine-tuning, product development, or competitive intelligence -- must obtain a **Data Use License** from the DANIELS AI maintainers.
...
By using DANIELS AI, you agree that:

- **You are responsible for the outputs generated through this tool.** DANIELS AI is a research interface -- it does not generate content itself, it routes requests to AI models via third-party APIs.
...
- **You will not use DANIELS AI to generate content that is illegal** in your jurisdiction.
...
DANIELS AI IS PROVIDED "AS-IS" WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.

The maintainers and contributors of DANIELS AI:
...
DANIELS AI interacts with third-party services. Your use of these services is governed by their respective terms:
...
DANIELS AI is not affiliated with, endorsed by, or sponsored by any of these services.

---

## 10. Open Source

DANIELS AI is licensed under the **GNU Affero General Public License v3.0 (AGPL-3.0)**.
...
These terms may be updated as the project evolves. Changes will be committed to the repository and visible in the git history. There is no email notification system -- if you use DANIELS AI, check this file periodically.
...
_These terms exist to be honest with you, not to protect a corporation. DANIELS AI collects metadata to understand how the system performs. It never collects your messages, your keys, or your identity in the always-on tiers. The opt-in tier is exactly that -- opt-in. If you're a person, a researcher, a small team -- this is free. If you're a giant corporation making money off of open-source labor, we're asking you to contribute back. The code is open. Verify it yourself._
