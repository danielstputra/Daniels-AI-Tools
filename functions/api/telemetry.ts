...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  if (!env.HF_TOKEN || !env.HF_DATASET_REPO) {
    ...
    console.error(
      `[Telemetry] Missing env vars: ${missing.join(", ")} — set these in Cloudflare Pages Dashboard → Settings → Environment Variables`,
    );
    return jsonResponse(
      { error: `Telemetry not configured (missing: ${missing.join(", ")})` },
      503,
    );
  }
...
async function commitToHF(
  ...
  const ndjson = [
    JSON.stringify({
      key: "header",
      value: { summary: `[telemetry] ${filePath}` },
    }),
    JSON.stringify({
      key: "file",
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401 || res.status === 403) {
        console.error(
          `[Telemetry] HF AUTH FAILED (${res.status}) — HF_TOKEN is invalid or lacks write access to "${repo}"`,
        );
      } else if (res.status === 404) {
        console.error(
          `[Telemetry] HF REPO NOT FOUND (404) — "${repo}" does not exist on HuggingFace`,
        );
      } else {
        console.error(
          `[Telemetry] HF commit failed (${res.status}): ${err.slice(0, 300)}`,
        );
      }
    }
