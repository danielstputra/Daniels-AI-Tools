...
/**
 * Auto-detect a self-hosted DANIELS AI API server at the same origin.
 *
 ...
 * is open (no DANIELSAI_API_KEY configured), proxy mode activates
  ...
  useEffect(() => {
    ...
    async function detect() {
      try {
        ...
        // Step 2: Check if auth is open (no DANIELSAI_API_KEY required)
        ...
        if (tierRes.ok || tierRes.status === 429) {
          ...
          console.log(
            "[DANIELS AI] Self-hosted API detected (open auth) at",
            origin,
          );
          ...
          setUltraplinianApiUrl(origin);
          console.log(
            "[DANIELS AI] Self-hosted API detected (auth required) at",
            origin,
          );
        }
      } catch {
