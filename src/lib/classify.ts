...
const RULES: PatternRule[] = [
...

  {
    ...
    patterns: [
      ...
      /\b(dan|do\s*anything\s*now|developer\s*mode|god\s*mode|daniels\s*ai)\b.{0,10}\b(prompt|mode|enable)\b/i,
    ],
  },

  {
    domain: "meta",
    sub: "prompt_injection",
    weight: 6,
    patterns: [
...
export function classifyPrompt(prompt: string): ClassificationResult {
  ...
  let best = {
    domain: "benign" as HarmDomain,
    sub: "other" as HarmSubcategory,
    total: 0,
    maxWeight: 0,
  };
  for (const entry of scores.values()) {
    if (
      ...
      (entry.total === best.total && entry.maxWeight > best.maxWeight)
    ) {
      best = entry;
    }
  }
  ...
  const hasHarmful = [...scores.values()].some(
    (s) => s.domain !== "benign" && s.domain !== "gray" && s.domain !== "meta",
  );
